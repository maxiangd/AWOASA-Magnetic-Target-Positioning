%%% Fitness Function
%%% x is a dynamic variable ranging from [-3, 3]

function result= Fitness_st(TA,InputMatrix)
pi=3.14;
...
% Theoretical magnetic moment components of target
MA=[435 115 780]; 
...
for i=1:6  
    for j=i+1:7
       ...
       B_i_noise=B_i+0.016*randn(size(B_i)); % Add Gaussian white noise
       B_j=U*alpha_j*(beta_j*Rj'-MA');
       B_j_noise=B_j+0.016*randn(size(B_j)); % Add Gaussian white noise
       ...
       B_deta_p(i,j)=B_i_noise-B_j_noise;  % Theoretical magnetic anomaly with noise
       
       B_deta_x_p(i,j)=B_x_i_noise-B_x_j_noise;% Estimated magnetic anomaly
       
       result=result+(B_deta_x_p(i,j)-B_deta_p(i,j))^2;  % Fitness calculation
