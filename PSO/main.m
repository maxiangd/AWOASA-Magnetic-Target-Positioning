...
for run_idx = 1:Run_times
    [Best_score,Best_pos,PSO_cg_curve] = PSO(SearchAgents_no,Max_iteration,lb,ub,dim,fobj);
    all_best_scores(run_idx) = Best_score;
    all_cg_curves(:,run_idx) = PSO_cg_curve;
end

% Calculate mean and std
score_variance = std(all_best_scores);
...
display(['Mean and std: ', num2str(score_mean), ' (', num2str(score_variance), ')']);
